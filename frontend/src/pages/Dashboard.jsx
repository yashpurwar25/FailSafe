...
const NeonCard = ({ title, value, trend, color, icon: Icon }) => (
  <motion.div 
    whileHover={{ y: -5 }}
    className={`glass-card p-6 rounded-3xl relative overflow-hidden border-l-4 ${color}`}
  >
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-12">
        ...
        <button onClick={load} className="p-3 glass-card rounded-2xl text-slate-400 hover:text-white transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-card p-8 rounded-[2.5rem]">
          <h3 className="text-white font-bold text-sm uppercase tracking-widest mb-6">Risk Density</h3>
          <div className="space-y-4">
            {stats?.risk_level_distribution ? (
              Object.entries(stats.risk_level_distribution).map(([level, count]) => (
                <div key={level} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5">
